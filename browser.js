    ...
    module.exports = {
        init: function (window, undefined) {
            ...
            var html = (function () {
                ...
                if (window && window.document && window.document.getElementByTagName) {
                    return (window.document.getElementByTagName('html') || [])[0];
                }
                return null;
            })();
            ...
            (function setBrowserInfo(browser) {
                if (window && window.navigator && html) {
                    ...
                    var setFlag = function (browser, html, prop) {
                        browser[prop] = true;
                        html.className = html.className + ' ' + prop;
                    };
            ...
            (function setOsInfo(browser) {
                if (window && window.navigator && html) {
                    ...
                    if (browser.os.isMac) {
                        html.className = html.className + ' mac';
                    } else if (browser.os.isWin) {
                        html.className = html.className + ' win';
                    }
                }
            })(browser);
            ...
            // You need to add CSS support to make it work (https://github.com/jo-asakura/less-mixins)
            // ----------------
            ...
            (function initFeatures(browser) {
                if (window && window.navigator && html) {
                    ...
                    html.className = html.className +
                        ' ' + (browser.features.sessionStorage ? 'sessionStorage' : 'no-sessionStorage') +
                        ' ' + (browser.features.touchDevice ? 'touch' : 'no-touch') +
                        ' ' + (browser.features.html5history ? 'html5history' : 'no-html5history') +
                        ' ' + (browser.features.isMobileDevice ? 'mobile' : 'desktop');
                }
            })(browser);
